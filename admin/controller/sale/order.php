...
**	Type				: lets BizTalk, baby!. Order Item Module.
...
class ControllerSaleOrder extends Controller {
	...
	public function info() {
      ...
      $data['button_sellercommission_add'] = $this->language->get('button_sellercommission_add');
			$data['button_sellercommission_remove'] = $this->language->get('button_sellercommission_remove');
      ...
      $data['seller_firstname'] = $seller_info['firstname'];
			...
			if ($order_info['seller_id']) {
				$data['seller'] = $this->url->link('seller/seller/edit', 'token=' . $this->session->data['token'] . '&seller_id=' . $order_info['seller_id'], true);
			} else {
				$data['seller'] = '';
			}

			$data['seller_commission'] = $this->currency->format($order_info['seller_commission'], $order_info['currency_code'], $order_info['currency_value']);

			$data['seller_commission_total'] = $this->model_seller_seller->getTotalTransactionsByOrderId($this->request->get['order_id']);
  ...
  public function addSellerCommission() {
		$this->load->language('sale/order');
		...
		if (!$this->user->hasPermission('modify', 'sale/order')) {
			...
			if ($order_info) {
				...
				$seller_total = $this->model_seller_seller->getTotalTransactionsByOrderId($order_id);

				if (!$seller_total) {
					$this->model_seller_seller->addTransaction($order_info['seller_id'], $this->language->get('text_order_id') . ' #' . $order_id, $order_info['seller_commission'], $order_id);
				}
			}
	...
	public function removeSellerCommission() {
		$this->load->language('sale/order');
		...
		if (!$this->user->hasPermission('modify', 'sale/order')) {
			...
			if ($order_info) {
				...
				$this->model_seller_seller->deleteTransaction($order_id);
			}
